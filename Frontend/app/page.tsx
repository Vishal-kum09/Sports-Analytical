...
// Plotly Fix for Next.js
const Plot = dynamic(() => import('react-plotly.js'), { 
  ssr: false,
  loading: () => <div className="h-64 bg-slate-900 animate-pulse rounded-2xl flex items-center justify-center text-slate-500">Loading Analytics Chart...</div>
});
...
export default function SportsDashboard() {
  ...
  // Agar data load hone ke baad bhi khali hai (Safety Check)
  if (!data || !data.summary) return (
    <div className="h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-4">
      <p className="text-xl font-bold mb-4 italic text-rose-500">Backend Connection Error</p>
      <p className="text-slate-400 mb-6 text-sm">Please refresh the page in 10 seconds.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-10">
      <div className="max-w-7xl mx-auto">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-slate-500 text-xs uppercase tracking-tighter">
                                ...
                                <th className="py-2 px-2 text-right text-emerald-400">SR</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/50">
                            {data.batting_leaderboard?.map((player: any) => (
                                <tr key={player.name} className="hover:bg-slate-800/30 transition">
                                    <td className="py-4 px-2 font-bold text-slate-200 text-sm">{player.name}</td>
                                    <td className="py-4 px-2 text-center font-mono">{player.runs}</td>
                                    <td className="py-4 px-2 text-right font-black text-emerald-400 font-mono">{player.sr}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-slate-500 text-xs uppercase tracking-tighter">
                                ...
                                <th className="py-2 px-2 text-right text-rose-400">Econ</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/50">
                            {data.bowling_leaderboard?.map((player: any) => (
                                <tr key={player.name} className="hover:bg-slate-800/30 transition">
                                    ...
                                    <td className="py-4 px-2 text-right font-black text-rose-400 font-mono">{player.economy}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
                ...
                <Plot
                    ...
                    layout={{ autosize: true, paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: {color: '#94a3b8'}, margin: {t:10, b:40, l:40, r:10} }}
            ...
            <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><BarChart3 size={18} className="text-purple-400"/> Pitch Map</h3>
                <Plot
                    data={[{ 
                        labels: data.length_data ? Object.keys(data.length_data) : [], 
                        values: data.length_data ? Object.values(data.length_data) : [], 
                        type: 'pie', hole: 0.5, marker: {colors: ['#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ef4444']} 
                    }]}
                    layout={{ autosize: true, paper_bgcolor: 'rgba(0,0,0,0)', font: {color: '#94a3b8'}, margin: {t:20, b:20, l:20, r:20}, showlegend: true }}
                    style={{ width: "100%", height: "300px" }}
                    config={{ responsive: true, displayModeBar: false }}
                />
            </div>
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><PieChart size={18} className="text-amber-400"/> Shot Selection</h3>
            <Plot
                data={[{ 
                    labels: data.shot_data ? Object.keys(data.shot_data) : [], 
                    values: data.shot_data ? Object.values(data.shot_data) : [], 
                    type: 'pie', marker: {colors: ['#2563eb', '#059669', '#d97706', '#dc2626', '#7c3aed']} 
                }]}
                layout={{ autosize: true, paper_bgcolor: 'rgba(0,0,0,0)', font: {color: '#94a3b8'}, margin: {t:20, b:20, l:20, r:20}, showlegend: true }}
